...
2.  Playground를 열고 LLM에게 귀하의 이름을 말해 주세요.
...
Language Model과 Agent 컴포넌트에는 기본적으로 활성화되어 있는 내장 채팅 메모리가 있으며, 이는 기능적으로 Message History 컴포넌트와 동일합니다. 외부 채팅 메모리 데이터베이스에 채팅 메모리를 저장하거나 불러오고 싶을 때, 혹은 채팅이 아닌 플로우에서 사용하거나 다른 채팅의 기억을 다른 세션에 제공하는 경우와 같이 현재 세션 컨텍스트 외부의 채팅 메모리를 가져와야 할 때만 Message History 컴포넌트를 사용하세요. 자세한 내용은 Store chat memory를 참조하세요.
